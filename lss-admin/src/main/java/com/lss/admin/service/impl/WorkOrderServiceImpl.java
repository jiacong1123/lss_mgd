		...
		if (params.getStatus() == 98) {// 待调库（回收）列表
			if (params.getStartCloseTime() != null) {
				...
				if (params.getEndCloseTime() != null) {
					params.setEndCloseTime(
							DateUtils.addDate(params.getEndCloseTime(), SystemConstant.WORK_ORDER_WILL_CLOSE_DAY));
				}
			} else {
...
//		if (params.getStatus() != 0 && params.getStatus() != 5) {//关闭的也只看本人的 2019.08.12 v1.4
		...
		// 标签查询
		if (params.getTag() != null && params.getTag() != "") {
			String[] split = params.getTag().split(",");
		...
		} else if (params.getStatus() == 5) {
			sort = " a.followuptime DESC "; // 掉库日期根据跟进日期降序排序
		} else {
		...
		if (!onlyCount) {// 查列表
			...
			// && params.getStatus() != 5 //关闭的也只看本人的 2019.08.12 v1.4
				...
				if (admin.getRoles().contains(3)) {
					...
					WorkTag tag = MapperManager.workTagMapper.queryByType(WorkTagType.ztc_url.getType());
					String json = null;
					try {
						// 2.从直通车查好友
						json = HttpUtil.post(tag.getTagname() + "/api-os/lss/gd/pmlist.do", map);
						...
						// 3.查完后组装到返回的list
						List<ZtcPersonMember> pMember = JSON.parseArray(obj.getString("returnObject"),
								ZtcPersonMember.class);
	...
	public ReturnVo details(String orderno) {
		ReturnVo result = new ReturnVo();
		WorkOrderDetails vo = MapperManager.workOrderMapper.queryDetails(orderno);
		if (vo != null) {
			if (vo.getClinicid() != null) {
				Clinic clinic = MapperManager.clinicMapper.selectByPrimaryKey(vo.getClinicid());
				vo.setClinicname(clinic.getShortname());
			}
			if (vo.getProjectid() != null) {
				WorkTag tag = MapperManager.workTagMapper.selectByPrimaryKey(vo.getProjectid());
				vo.setProject(tag.getTagname());
			}
			if (vo.getAdminid() != null) {
				Admin admin = MapperManager.adminMapper.selectByPrimaryKey(vo.getAdminid());
				vo.setAdminname(admin.getName());
			}
			if (vo.getDoctorid() != null) {
				Admin admin = MapperManager.adminMapper.selectByPrimaryKey(vo.getDoctorid());
	...
	public ReturnVo userinfo(String phone, String noWxAlias) {
		...
		if (user != null) {
			// 用户信息存在。。查询是否有未完成的工单
			WorkOrder order = MapperManager.workOrderMapper.queryWorkOrder(user.getUserid());
...
//				userOrder.setAdminname(adminname);
	...
	public ReturnVo save(UserOrderParams params, LoginAdmin admin) {
		...
		if (ObjectUtil.isEmpty(order.getOrderno())) {
			...
			WorkOrder temp = MapperManager.workOrderMapper.selectByPrimaryKey(order.getOrderno());
			...
			if (temp.getStatus() == 0 && order.getAdminid() != null) {
				insertWorkRecord(order.getOrderno(), admin.getAdminid(), "分配", null);
			}
		}
		result.setResult(ResponseCode.success);
		result.setMsg(ResponseCode.successMsg);
	...
	public ReturnVo batchassign(WorkOrderParams params, LoginAdmin admin) {
		ReturnVo result = new ReturnVo();
		// 2019-11-06 新需求:只有电销组长才能分配客户
		if (!admin.getRoles().contains(9)) {
			result.setResult(ResponseCode.noPermission);
			result.setMsg("非电销组长,不允许分配客户");
			return result;
		}
		if (params.getAdminid() == null) {
			result.setResult(ResponseCode.parameterError);
		...
		for (String orderno : orderonArr) {
			...
			if (MapperManager.workOrderMapper.closeWorkOrder(updateOrder) > 0) {
				...
				insertWorkRecord(orderno, admin.getAdminid(),
						"关闭工单！关闭原因：" + (null == order.getClosereason() ? "" : order.getClosereason()),
						order.getFollowup());
				result.setResult(ResponseCode.success);
				result.setMsg(ResponseCode.successMsg);
			} else {
	...
	public ReturnVo followup(WorkOrder order, LoginAdmin admin) {
		 ...
		 * .selectByPrimaryKey(order.getOrderno()); /* if
		 * (!admin.getAdminid().equals(workOrder.getAdminid())) {
		 * result.setResult(ResponseCode.failure); result.setMsg("非法操作，您不能跟进该工单！");
		 * return result; }
		 */
		order.setDoctorid(0);
	...
	public ReturnVo reserve(WorkOrder order, LoginAdmin admin) {
		 ...
		 * .selectByPrimaryKey(order.getOrderno()); /* if
		 * (!admin.getAdminid().equals(workOrder.getAdminid())) {
		 * result.setResult(ResponseCode.failure); result.setMsg("非法操作，您不能预约该工单！");
		 * return result; }
		 */
	 ...
	 */
	@Override
	public Integer insertWorkRecord(String orderno, Integer adminid, String content, Integer talktime) {
		Date now = new Date();
	...
	public ReturnVo transaction(WorkOrder order, LoginAdmin admin) {
		...
		if (MapperManager.workOrderMapper.transaction(order) > 0) {
			...
			insertWorkRecord(order.getOrderno(), admin.getAdminid(),
					"已成交！" + (null == order.getWorknotes() ? "" : order.getWorknotes()), null);
	...
	public ReturnVo activation(WorkOrder order, LoginAdmin admin) {
		...
		for (String orderno : ordernos) {
			...
			if (MapperManager.workOrderMapper.activation(workOrder) > 0) {
				insertWorkRecord(workOrder.getOrderno(), admin.getAdminid(), "激活工单", null);
				result.setResult(ResponseCode.success);
				result.setMsg(ResponseCode.successMsg);
			} else {
	...
	public ReturnVo deleteWorkOrder(WorkOrder order, LoginAdmin admin) {
		...
		if (i > 0) {
			// 添加记录
			insertWorkRecord(order.getOrderno(), admin.getAdminid(), "删除工单！" + admin.getName(), order.getFollowup());
