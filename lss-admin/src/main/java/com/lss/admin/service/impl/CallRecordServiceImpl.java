...
public class CallRecordServiceImpl implements ICallRecordService {
	...
	public ReturnVo findCallRecordPage(FindCallRecordPage findCallRecordPage) throws LssException {
		...
		try {
			...
			if (null != findCallRecordPage.getStartDate()) {
				String start = DateUtils.format(findCallRecordPage.getStartDate(), "yyyy-MM-dd");
				findCallRecordPage.setStartDate(sft.parse(start + " 00:00:00"));
			}
			if (null != findCallRecordPage.getEndDate()) {
				String end = DateUtils.format(findCallRecordPage.getEndDate(), "yyyy-MM-dd");
				findCallRecordPage.setEndDate(sft.parse(end + " 23:59:59"));
			}
		} catch (ParseException e) {
			logger.error("查询电话记录开始/结束时间转换错误", e);
			e.printStackTrace();
		}
		List<CallRecordDto> returnList = null;
		int count = 0;
